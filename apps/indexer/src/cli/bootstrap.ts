...
async function completeDatabaseBootstrap(manifest: DeploymentManifest) {
  ...
  try {
    ...
    if (vehicles.data.length !== 4 || !sale.data) throw new Error('BOOTSTRAP_SCENARIO_INCOMPLETE');
...
if (existsSync(config.manifestPath)) {
  ...
  if (
    ...
    throw new Error('DEPLOYMENT_MISMATCH: run pnpm demo:reset --yes before replacing local state');
