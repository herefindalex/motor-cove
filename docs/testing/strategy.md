...
flowchart BT
  ...
  E2E[Playwright browser E2E with test wallet adapter]
...
- **E2E Tests:** Playwright through the local browser stack and controlled EIP-1193 provider.
...
| Indexer replay                           | Real SQLite and selected Anvil fixtures                           | Event identity, overlap idempotency, atomic projection/checkpoint commit, rebuild, selected canonical reindex and reorg recovery | Unavailable archive history or public-network finality policy |
...
| Browser E2E                              | Playwright, local stack, explicit test EIP-1193 adapter           | UI-to-local-chain paths, reload, rejection, account/network changes, stale/catch-up behavior                                     | MetaMask or broad provider compatibility                      |
...
| Is wallet rejection visible without becoming a submitted transaction?              | `TransactionTimeline.test.tsx` with React Testing Library plus the Playwright rejection scenario                                  | `RUN-WEB-COMPONENT` and `RUN-E2E`: executed/pass                                                | Automated provider is test-only; manual MetaMask remains not run                                                                            |
...
| Does a real end-to-end settlement reach the query API?                             | `tests/integration/real-stack.test.ts` and `tests/e2e/marketplace.spec.ts`                                                        | `RUN-INTEGRATION` and `RUN-E2E`: executed/pass                                                  | Uses local Anvil and a test wallet adapter                                                                                                  |
...
| Are feature/capability/adapter and server package directions enforced?             | `tooling/architecture/check.mjs` plus seven negative fixtures                                                                     | `RUN-ARCHITECTURE`: executed/pass                                                               | Static source imports only                                                                                                                  |
