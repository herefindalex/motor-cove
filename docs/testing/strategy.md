...
| Can bootstrap overlap a standard backup or restore into mixed database and sidecar generations?    | Real bootstrap `flock`, better-sqlite3 backup/restore fixtures, incomplete sidecar pairs, and pre-quarantine generation comparison                               | `RUN-DB-RECOVERY` and `RUN-DB-MIGRATIONS`                                                         | Same-host advisory locks and local filesystem; hardware loss remains outside the claim                                    |
| Can reconciliation publish stale `CURRENT` after a long anchored comparison?                       | Production CLI with real SQLite and controlled initial, anchored, and publication-time head reads                                                                | `RUN-INTEGRATION`: head movement becomes `PROJECTION_LAGGING`; failed publication read is unknown | Controlled chain client; real Anvil timing is covered by the broader reconciliation lane                                  |
| Can reset mutate the chain without durable recovery intent?                                        | Reset CLI marker-before-RPC assertion, real child `SIGKILL` inside the chain callback, filesystem cleanup failure, and managed Anvil integration                 | `RUN-DB-BOUNDARIES` and `RUN-INTEGRATION`: PREPARED stays actionable; CHAIN_RESET cleanup resumes | Child-kill chain side effect is controlled; the real Anvil integration runs the normal reset path without fault injection |
| Can a stricter indexing depth leave held-back blocks marked current?                               | `ingest-range.test.ts` with checkpoint 100, head 100, depth increase 0→10, depth decrease, unchanged depth, and no-eligible-block cases                          | `RUN-INDEXER-UNIT`: increased depth requires recovery; decreased depth catches forward            | Controlled chain port; explicit reindex behavior is covered by existing maintenance and canonical reindex suites          |
