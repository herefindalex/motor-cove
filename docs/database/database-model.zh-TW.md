...
| 身分     | `id`；`deployment_id` 與 `run_sequence` 決定同一部署內的報告順序                       |
| 作家     | 對帳 CLI                                                                               |
| 讀者     | API診斷；操作員                                                                        |
| 衍生     | 是的                                                                                   |
| 可重建   | 不                                                                                     |
...
| 生命週期 | 比對結果確定後附加報告；在對帳 maintenance ownership 下以交易方式配置 `run_sequence`。 |
...
`created_at` 是人類可讀的發布時間；最新報告依 `run_sequence` 選取，不依牆上時間。報告不會繼承之後的檢查點、範圍或鏈頭。
