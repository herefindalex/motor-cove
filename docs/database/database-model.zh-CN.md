...
| 身份     | `id`；`deployment_id` 与 `run_sequence` 决定同一部署内的报告顺序                       |
| 作家     | 对账 CLI                                                                               |
| 读者     | API诊断；运营商                                                                        |
| 派生     | 是的                                                                                   |
| 可重建   | 不                                                                                     |
...
| 生命周期 | 比对结果确定后追加报告；在对账 maintenance ownership 下以事务方式分配 `run_sequence`。 |
...
`created_at` 是供人阅读的发布时间；最新报告按 `run_sequence` 选取，不按墙上时间。报告不会继承之后的检查点、范围或链头。
