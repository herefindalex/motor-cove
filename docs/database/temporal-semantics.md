...
| `reconciliation_runs`    | `created_at`; `run_sequence`; comparison block/hash and recorded head | `run_sequence` is the deployment-scoped publication order. `created_at` is human publication time and cannot select the latest report. Anchors and scope remain historical. |

Migration `0002_reconciliation_sequence` backfills older reports from the available SQLite row order within each deployment. That order is only a best-effort legacy proxy; the old schema did not preserve a separate publication sequence, and earlier database rewrites could have changed rowids. New reports allocate `run_sequence` transactionally under reconciliation maintenance ownership. The reader selects the latest report by sequence.
