...
## 公链 profile 的已包含与已达最终性

Ethereum 与 Polygon 的投影可索引范围依链 profile 的 finalized RPC 证据决定。成功且已包含的收据仍是交易证据；它本身不会使 Sale 立即出现在已达最终性的应用投影。Anvil 是 loopback 立即最终性 profile。Finalized 锚点若发生矛盾，必须进入恢复，不能只当作普通浅层 reorg 重试。新 listing 的不可变意图与合约调用包含卖家选定的 `allowedBuyer`；只有该地址成功付款后，才设置实际 `buyer`。
