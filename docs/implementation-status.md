...
| Frontend  | Feature/capability/adapter layers, exact bigint amount presentation, explicit injected local connector states, durable pre-submit journal, read-only transaction recovery | `apps/web` capability ports plus EVM, HTTP, Wagmi, and localStorage adapters; full deployment recheck after the awaited pre-wallet write; per-operation journal revisions; Web Locks serialize journal writers; client routing preserves the tab overlay | All eight supported action receipt outcomes; a context change while persistence waits keeps wallet calls at zero; stale revisions fail explicitly; volatile hash navigation and real reload limits are exercised. Manual MetaMask was not run.         |
...
- `pnpm verify`: generation, database contract, formatting, docs, architecture, type checking,
  lint, 13 Foundry tests, 220 unit/component/database tests, 47 integration tests, and all
