...
| Frontend  | Feature/capability/adapter layers, exact bigint amount presentation, explicit injected local connector states, durable pre-submit journal, read-only transaction recovery | `apps/web` capability ports plus EVM, HTTP, Wagmi, and localStorage adapters                                                                                                                    | Unit, integration, and Playwright coverage verifies all supported action receipts, identity mismatch rejection, known-hash inspection during storage failure, unchanged durable bytes, tab-local warnings, reload fallback, and zero added wallet submissions. Manual MetaMask was not run.                               |
...
R17 hardening is implemented in the working source. The published marker is the only committed
maintenance state, so an unpublished temporary file cannot block a matching resume. Backup format 3
records source readiness and database-derived projection evidence. Maintenance snapshots are
operation-bound `EVIDENCE_ONLY` archives; normal restore accepts only `STANDARD` ready-source
snapshots.

- `pnpm verify`: generation, database contract, formatting, docs, architecture, type checking, lint,
  13 Foundry tests, 44 unit/component/database files with 295 tests, 14 integration files with 58
  tests, and all 7 workspace builds passed.
- `pnpm test:e2e`: 6 local Playwright scenarios passed against harness-owned Anvil, managed SQLite,
  API, Indexer, and Web.
- Focused R17 coverage passed marker, recovery, migration, and source-refresh integration paths. The
  marker case uses a real child process and `SIGKILL`; backup mutation and failure boundaries use
  harness-owned SQLite environments.
