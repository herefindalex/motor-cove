...
| Frontend  | Feature/capability/adapter layers, exact bigint amount presentation, explicit injected and local connector states, durable pre-submit journal, and read-only transaction recovery | `apps/web` capability ports plus EVM, HTTP, Wagmi, and localStorage adapters; generation-guarded verification; Web Locks serialize journal writers per deployment         | All eight supported actions have receipt outcomes; exact visible/send values, stale observation presentation, and non-funding reload cases passed. Six Playwright scenarios include stopped-Indexer and real multi-tab cases. Manual MetaMask was not run. |
| API       | Read-only query API with validated wire schemas, uint256 request boundaries, historical report scope, observation freshness, and provenance                                       | Database reader plus Zod and OpenAPI contracts                                                                                                                            | 17 Fastify/reader contract tests across two files, read-only boundaries, one-snapshot concurrency, stale-heartbeat semantics, and real local convergence passed.                                                                                           |
...
- `pnpm verify`: generation, database contract, formatting, docs, architecture, type checking,
  lint, 13 Foundry tests, 203 unit/component/database tests, 46 integration tests, and all
...
- `pnpm test:e2e`: 6 local Playwright scenarios passed against Anvil, managed SQLite, API,
  ...
  multi-tab journal coordination and writer handoff. A fresh context also observes a stopped
  Indexer as stale without relying on local transaction evidence.
...
- Architecture validation passed for 149 source files and rejected all 8 negative fixtures.
