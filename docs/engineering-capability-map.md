...
| CAP-14 | Release management | partial | RUN-GENERATE-CHECK: executed/pass<br>RUN-CI: not run | Local and remote gates are commit-specific; the verification record must name the exact source context and remote run instead of reusing an older source claim. |
